...
public class TagletWorklet implements Taglet {
    
    public TagletWorklet() {
    }
    ...
    static protected void addTaglet(Map tagletMap, TagletWorklet taglet) {
    ...
    public static void register(Map tagletMap) {
        addTaglet(tagletMap, new TagletWorklet());
    }

    public Hashtable getMetaData(Tag tag) {
        Hashtable result = new Hashtable();
        for (Tag it : tag.inlineTags()) {
            if (it.name().matches("@(?i:Category|SecurityClass|Language|Audience|Maturity|Intent|Extent|Form|Encoding|Aspect)")) {
